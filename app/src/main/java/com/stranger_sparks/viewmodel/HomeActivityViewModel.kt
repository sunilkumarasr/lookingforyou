...
class HomeActivityViewModel@Inject constructor(private val strangerSparksRepository: StrangerSparksRepository) : ViewModel() {
    ...
    private val _versionControl = MutableLiveData<SuggestionVersionResponse?>()
    val versionControl: LiveData<SuggestionVersionResponse?> get() = _versionControl
    ...
    fun getVersionControl(version: String) {
        strangerSparksRepository.getVersionControl(version).enqueue(object :
            Callback<SuggestionVersionResponse> {
            override fun onResponse(call: Call<SuggestionVersionResponse>, response: Response<SuggestionVersionResponse>) {
                if (response.isSuccessful) {
                    _versionControl.value = response.body()
                } else {
                    _versionControl.value =null
                }
            }

            override fun onFailure(call: Call<SuggestionVersionResponse>, t: Throwable) {
                _versionControl.value = null
            }
        })
    }
